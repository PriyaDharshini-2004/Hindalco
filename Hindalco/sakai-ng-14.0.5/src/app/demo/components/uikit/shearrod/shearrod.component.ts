...
@Component({
  selector:'pump-1',
  templateUrl: './shearrod.component.html'
})
export class ShearrodComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getShearrodDataGraph().subscribe(
      (result: any) => {
        ...
        const data = result.map((record: any) => record.shear_cylinder_rod);

        this.data = {
          labels: dates, // Use dates as labels
          datasets: [
            {
              label: 'Shear Cylinder Rod Data',
              data: data,
              fill: false,
