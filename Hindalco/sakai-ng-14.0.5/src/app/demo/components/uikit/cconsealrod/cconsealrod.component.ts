...
@Component({
  selector:'cconsealrod-1',
  templateUrl: './cconsealrod.component.html'
})
export class CconsealrodComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getConsealrodDataGraph1().subscribe(
      (result: any) => {
        ...
        const data = result.map((record: any) => record.conseal_cylinder_rod);

        this.data = {
          labels: dates, // Use dates as labels
          datasets: [
            {
              label: 'Conseal Cylinder rod Data',
              data: data,
              fill: false,
