...
@Component({
  selector:'ssidebore-1',
  templateUrl: './ssidebore.component.html'
})
export class SsideboreComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getSideboreDataGraph1().subscribe(
      (result: any) => {
        ...
        const data = result.map((record: any) => record.side_cylinder_bore);

        this.data = {
          labels: dates, // Use dates as labels
          datasets: [
            {
              label: 'Side Cylinder Bore Data',
              data: data,
              fill: false,
