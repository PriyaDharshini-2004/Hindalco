...
@Component({
  selector:'ssiderod-1',
  templateUrl: './ssiderod.component.html'
})
export class SsiderodComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getSiderodDataGraph1().subscribe(
      (result: any) => {
        ...
        const data = result.map((record: any) => record.side_cylinder_rod);

        this.data = {
          labels: dates, // Use dates as labels
          datasets: [
            {
              label: 'Side Cylinder Rod Data',
              data: data,
              fill: false,
