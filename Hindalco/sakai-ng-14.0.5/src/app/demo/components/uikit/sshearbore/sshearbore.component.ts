...
@Component({
  selector:'sshearbore-1',
  templateUrl: './sshearbore.component.html'
})
export class SshearboreComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getShearboreDataGraph1().subscribe(
      (result: any) => {
        ...
        const data = result.map((record: any) => record.shear_cylinder_bore);

        this.data = {
          labels: dates, // Use dates as labels
          datasets: [
            {
              label: 'Shear Cylinder Bore Data',
              data: data,
              fill: false,
