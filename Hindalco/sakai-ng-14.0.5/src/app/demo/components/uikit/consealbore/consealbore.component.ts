...
@Component({
  selector:'pump-1',
  templateUrl: './consealbore.component.html'
})
export class ConsealboreComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getConsealboreDataGraph().subscribe(
      (result: any) => {
        ...
        const data = result.map((record: any) => record.conceal_cylinder_rod);

        this.data = {
          labels: dates, // Use dates as labels
          datasets: [
            {
              label: 'Conseal Cylinder Bore Data',
              data: data,
              fill: false,
